import os
import logging
from pyspark.sql.functions import col, current_timestamp, to_date, concat, lit, year, coalesce
...
def transform_bronze_to_silver(spark):
    ...
    df_silver = df.select(
        ...
        coalesce(col("nombre").cast(IntegerType()), col("complement_info_nombre").cast(IntegerType())).alias("nombre"),
        # Règle Métier : Idem pour le taux (redressement statistique)
        coalesce(col("taux_pour_mille").cast(DoubleType()), col("complement_info_taux").cast(DoubleType())).alias("taux_pour_1000"),
    ...
    df_silver = df_silver.filter(
        (col("code_geo").rlike("^6938[1-9]$")) &
        (year(col("date_reference")).between(2017, 2022))
    )
