...
def run_gold_securite_bi_pipeline():
    ...
    ).distinct().withColumn("nom_arrondissement",
    ...
    dim_indicateurs.write.mode("overwrite").parquet(os.path.join(output_base_path, "dim_indicateurs_securite"))
    dim_geographie.write.mode("overwrite").parquet(os.path.join(output_base_path, "dim_geographie_lyon"))
    fact_securite.write.mode("overwrite").parquet(os.path.join(output_base_path, "fact_securite"))
    fact_demographie.write.mode("overwrite").parquet(os.path.join(output_base_path, "fact_demographie_annuelle"))
    ...
    logger.info(f"Audit Gold : {nb_indicateurs} indicateurs sur {nb_arrondissements} arrondissements.")
